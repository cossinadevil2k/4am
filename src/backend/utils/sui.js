...
export async function send(browser, address, amount) {
  ...
  await suiPage.goto("chrome-extension://opcgpfmipidbgpenhmajoajpbobppdil/ui.html#/send?type=0x2::sui::SUI", { waitUntil: "networkidle0" })
  await sleep(500)
...
export async function disconnect(browser, name) {
  const suiPage = await browser.newPage()
  await suiPage.goto("chrome-extension://opcgpfmipidbgpenhmajoajpbobppdil/ui.html#/apps/connected", { waitUntil: "networkidle2" })
  const btn = await findElementByTypeAndText(suiPage, name, "div[role='button']")
  // 滚动到元素位置,防止不在视口内点击无效
  if (btn) {
...
export async function findSuiPage(browser, type, maxAttempts = 5) {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    const approval = targets.reverse().find((v) => v.url().includes(`chrome-extension://opcgpfmipidbgpenhmajoajpbobppdil/ui.html#${typeList[type]}`))
...
export async function auth(browser) {
  ...
  if (approval) {
    ...
    await btn.click()
    await sleep(1000)
    const mayFail = await findElementByTypeAndText(page, "This transaction might fail. Are you sure you still want to approve the transaction?", "div")
    if (mayFail) {
      await sleep(1000)
      page.close()
...
export async function connect(browser) {
  ...
  if (approval) {
    ...
    const connectBtn = await findElementByTypeAndText(approvalPage, "Connect", "button")
...
// 注册
export async function registry(browser, password) {
  const page = await browser.newPage()
  await page.goto("chrome-extension://opcgpfmipidbgpenhmajoajpbobppdil/ui.html#/initialize/create", { waitUntil: "networkidle0" })
  await sleep(500)
  const psw = await page.$("input[name='password']")
  const cpsw = await page.$("input[name='confirmPassword']")
  const check = await page.$("input#terms")
  const submit = await page.$("button[type='submit']")

  if (!psw) throw Error("psw not found")
  await psw.type(password)
  await cpsw.type(password)
  await check.click()
  await submit.click()
  await sleep(500)
  return page
