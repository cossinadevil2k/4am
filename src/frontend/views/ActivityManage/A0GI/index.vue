<template>
  <PageCard>
    <el-row slot="header-right">
      ...
      <el-button type="primary" size="mini" @click="open('add')" title="刷新列表">新增</el-button>
    </el-row>
    <el-table v-loading="loading" :data="tableData" style="width: 100%" height="100%" @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="name" label="名称" min-width="140">
        <template #default="{ row }">
          <el-button type="text" @click="$router.push({ name: 'ActivityManage_A0GI_Detail', query: { dbid: row.dbid, name: row.name } })">{{ row.name }}</el-button>
        </template>
      </el-table-column>
      <el-table-column prop="walletGroupName" label="钱包分组" min-width="140"> </el-table-column>
      ...
      <el-table-column prop="operate" label="操作">
        <template #default="{ row }">
          <el-button :loading="row.loading" type="text" size="mini" @click="$router.push({ name: 'ActivityManage_A0GI_Detail', query: { dbid: row.dbid, name: row.name } })">详 情</el-button>
          <el-button :loading="row.loading" type="text" size="mini" @click="open('add', row)">编 辑</el-button>
          <el-button :loading="row.loading" type="text" size="mini" @click="getLiquid(row)">0g领水</el-button>
          <el-button :loading="row.loading" type="text" size="mini" @click="getBalance(row)">刷新余额</el-button>
        </template>
      </el-table-column>
    </el-table>
...
import { list, remove, faucet,getBalance } from "@/api/a0gi"
